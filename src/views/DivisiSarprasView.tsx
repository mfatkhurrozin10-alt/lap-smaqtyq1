...
export default function DivisiSarprasView({ showNotification }: any) {
  ...
  const [formConfig, setFormConfig] = useState<any>({
    ...
    label_guru: 'Penanggung Jawab / Teknisi',
    label_mapel: 'Fasilitas / Barang',
    label_kelas: 'Lokasi / Ruang',
    label_jam: 'Waktu Pengecekan',
    label_santri_absen: 'Keterangan Kerusakan',
    type_petugas: 'input',
    type_guru: 'input',
    type_mapel: 'input',
    type_kelas: 'input',
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const divisiObj = divList?.find((d: any) => d.nama_divisi?.toLowerCase().includes('sarpras')) || divList?.[0];
    ...
    <div className="space-y-6 w-full text-left pb-12 font-sans text-slate-800">
      <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-0.5">Koordinator: <span className="font-semibold text-slate-700">{divisiData?.nama_koordinator || 'Tim Sarpras'}</span></p>
        </div>
          ...
          <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 pt-2">
              {[
                ...
                { key: 'show_guru', defaultLabel: 'Penanggung Jawab / Teknisi', labelKey: 'label_guru', typeKey: 'type_guru', optsKey: 'options_guru' },
                { key: 'show_mapel', defaultLabel: 'Fasilitas / Barang', labelKey: 'label_mapel', typeKey: 'type_mapel', optsKey: 'options_mapel' },
                { key: 'show_kelas', defaultLabel: 'Lokasi / Ruang', labelKey: 'label_kelas', typeKey: 'type_kelas', optsKey: 'options_kelas' },
                { key: 'show_jam', defaultLabel: 'Waktu Pengecekan', labelKey: 'label_jam', typeKey: 'type_jam', optsKey: 'options_jam' },
                { key: 'show_santri_absen', defaultLabel: 'Keterangan Kerusakan', labelKey: 'label_santri_absen', typeKey: 'type_santri_absen', optsKey: 'options_santri_absen' },
              ].map((item) => {
