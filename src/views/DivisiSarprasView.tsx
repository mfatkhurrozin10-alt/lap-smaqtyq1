...
export default function DivisiSarprasView({ showNotification }: any) {
  ...
  const [divisiData, setDivisiData] = useState<any>({ nama_divisi: 'Sarpras dan Bendahara', nama_koordinator: 'Tim Sarpras & Keuangan' });
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const targetDiv = divList?.find((d: any) => d.nama_divisi?.toLowerCase().includes('sarpras') || d.nama_divisi?.toLowerCase().includes('bendahara')) || divList?.[0];
      if (targetDiv) {
  ...
  const handleSubmit = async (e: any) => {
    ...
    try {
      ...
      showNotification('Laporan Sarpras & Bendahara berhasil disimpan!', 'success');
  ...
  if (loading) return <div className="p-12 text-center text-slate-500 font-medium">Memuat data Divisi Sarpras & Bendahara...</div>;
  ...
  return (
    <div className="space-y-6 w-full text-left pb-12 font-sans text-slate-800">
      <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm flex justify-between items-center">
        <div>
          <h2 className="text-xl sm:text-2xl font-bold text-slate-800">Divisi Sarpras dan Bendahara</h2>
      ...
      {activeSubTab === 'form' && (
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm space-y-4">
            <textarea value={formInput.catatan} onChange={e => setFormInput({...formInput, catatan: e.target.value})} placeholder="Catatan fasilitas / keuangan..." className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm outline-none h-24 resize-none" />
      ...
      {activeSubTab === 'riwayat' && (
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
          <div className="p-6 border-b border-slate-100 flex justify-between items-center"><h3 className="font-bold text-slate-800">Riwayat Sarpras & Bendahara</h3></div>
          <table className="w-full text-left text-sm whitespace-nowrap">
            <thead className="bg-slate-50 border-b border-slate-200 text-xs font-bold text-slate-500 uppercase">
              <tr><th className="px-6 py-4">Waktu</th><th className="px-6 py-4">Petugas</th><th className="px-6 py-4">Unit Terkait</th><th className="px-6 py-4">Skor</th></tr>
            </thead>
