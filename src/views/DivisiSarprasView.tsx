// src/views/DivisiSarprasView.tsx
...
export default function DivisiSarprasView({ showNotification }: any) {
  const [loading, setLoading] = useState(true);
  const [divisiData, setDivisiData] = useState<any>({
    nama_divisi: 'Sarpras',
    nama_koordinator: 'Tim Sarana & Prasarana'
  });
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const divisiObj = divList?.find((d: any) => d.nama_divisi?.toLowerCase().includes('sarpras') || d.nama_divisi?.toLowerCase().includes('sarana')) || divList?.[0];
      ...
      if (dbKelas && dbKelas.length > 0) {
        ...
        setKelasList(['Gedung A', 'Gedung B', 'Asrama Putra', 'Asrama Putri', 'Masjid', 'Kantor']);
      }
    } catch (err) {
      console.error(err);
    } finally {
  ...
  const handleSubmitCeklis = async (e: any) => {
    ...
    try {
      ...
      if (editingLogId) {
        ...
        if (showNotification) showNotification('Laporan sarpras berhasil diperbarui!', 'success');
        setEditingLogId(null);
      } else {
        const { error } = await supabase.from('divisi_log_pengawasan').insert([payload]);
        if (error) throw error;
        if (showNotification) showNotification('Laporan sarpras baru berhasil disimpan!', 'success');
      }
  ...
  if (loading) {
    return <div className="p-12 text-center text-slate-500 font-medium">Memuat data Divisi Sarpras...</div>;
  }
  ...
  return (
    <div className="space-y-6 w-full text-left pb-12 font-sans text-slate-800">
      <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h2 className="text-xl sm:text-2xl font-bold text-slate-800 tracking-tight">Divisi {divisiData?.nama_divisi || 'Sarpras'}</h2>
          <p className="text-sm text-slate-500 mt-0.5">Koordinator: <span className="font-semibold text-slate-700">{divisiData?.nama_koordinator || 'Tim Sarana & Prasarana'}</span></p>
        </div>
          ...
          <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm space-y-4">
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              <h3 className="font-bold text-slate-800 flex items-center gap-2">
                <CheckSquare size={18} className="text-blue-600" /> Informasi Pengawasan Sarpras & Penanggung Jawab
              </h3>
              {editingLogId && (
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              {formConfig.show_guru && (
                <div>
                  <label className="block text-xs font-semibold text-slate-600 mb-1.5">Penanggung Jawab Ruang / Aset:</label>
                  <select 
              ...
              {formConfig.show_mapel && (
                <div>
                  <label className="block text-xs font-semibold text-slate-600 mb-1.5">Jenis Aset / Fasilitas:</label>
                  <select 
              ...
              {formConfig.show_kelas && (
                <div>
                  <label className="block text-xs font-semibold text-slate-600 mb-1.5">Lokasi / Gedung:</label>
                  <select 
              ...
              {formConfig.show_jam && (
                <div>
                  <label className="block text-xs font-semibold text-slate-600 mb-1.5">Waktu Pengecekan:</label>
            ...
            <table className="w-full text-left text-sm whitespace-nowrap">
              <thead className="bg-slate-50 border-b border-slate-200 text-xs font-bold text-slate-500 uppercase tracking-wider">
                <tr>
                  ...
                  <th className="px-6 py-4">PJ Ruang</th>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-xs p-4 overflow-y-auto">
          <div className="bg-white rounded-3xl max-w-2xl w-full p-6 shadow-2xl border border-slate-200 space-y-6 relative animate-in fade-in zoom-in duration-200">
            ...
            <div className="flex items-center justify-between border-b border-slate-100 pb-4">
              <div>
                <h3 className="text-lg font-black text-slate-900 flex items-center gap-2">
                  <Eye size={20} className="text-blue-600" /> Detail Hasil Pengawasan Sarpras
                </h3>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              ...
              <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100 space-y-1">
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">ASET & LOKASI</p>
                <h4 className="text-xs font-bold text-slate-800 break-words">{selectedDetailLog.mapel_kelas}</h4>
              </div>
              <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100 space-y-1">
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">PJ RUANG</p>
                <h4 className="text-xs font-bold text-slate-800 leading-snug break-words">{selectedDetailLog.guru_target}</h4>
              </div>
              <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100 space-y-1">
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">KONDISI</p>
          ...
          <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm space-y-4">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3 pt-2">
              ...
              <label className={`flex items-center gap-2.5 p-3.5 rounded-xl border text-sm font-medium cursor-pointer transition-all ${formConfig.show_guru ? 'border-blue-500 bg-blue-50/20 text-blue-900 shadow-2xs' : 'border-slate-200 bg-slate-50 text-slate-600'}`}>
                <input type="checkbox" checked={formConfig.show_guru} onChange={e => handleToggleConfigField('show_guru', e.target.checked)} className="w-4 h-4 text-blue-600 rounded" /> Penanggung Jawab
              </label>
              <label className={`flex items-center gap-2.5 p-3.5 rounded-xl border text-sm font-medium cursor-pointer transition-all ${formConfig.show_mapel ? 'border-blue-500 bg-blue-50/20 text-blue-900 shadow-2xs' : 'border-slate-200 bg-slate-50 text-slate-600'}`}>
                <input type="checkbox" checked={formConfig.show_mapel} onChange={e => handleToggleConfigField('show_mapel', e.target.checked)} className="w-4 h-4 text-blue-600 rounded" /> Jenis Aset
              </label>
              <label className={`flex items-center gap-2.5 p-3.5 rounded-xl border text-sm font-medium cursor-pointer transition-all ${formConfig.show_kelas ? 'border-blue-500 bg-blue-50/20 text-blue-900 shadow-2xs' : 'border-slate-200 bg-slate-50 text-slate-600'}`}>
                <input type="checkbox" checked={formConfig.show_kelas} onChange={e => handleToggleConfigField('show_kelas', e.target.checked)} className="w-4 h-4 text-blue-600 rounded" /> Lokasi / Gedung
              </label>
              <label className={`flex items-center gap-2.5 p-3.5 rounded-xl border text-sm font-medium cursor-pointer transition-all ${formConfig.show_jam ? 'border-blue-500 bg-blue-50/20 text-blue-900 shadow-2xs' : 'border-slate-200 bg-slate-50 text-slate-600'}`}>
                <input type="checkbox" checked={formConfig.show_jam} onChange={e => handleToggleConfigField('show_jam', e.target.checked)} className="w-4 h-4 text-blue-600 rounded" /> Waktu Pengecekan
              </label>
              <label className={`flex items-center gap-2.5 p-3.5 rounded-xl border text-sm font-medium cursor-pointer transition-all ${formConfig.show_santri_absen ? 'border-blue-500 bg-blue-50/20 text-blue-900 shadow-2xs' : 'border-slate-200 bg-slate-50 text-slate-600'}`}>
                <input type="checkbox" checked={formConfig.show_santri_absen} onChange={e => handleToggleConfigField('show_santri_absen', e.target.checked)} className="w-4 h-4 text-blue-600 rounded" /> Kondisi
              </label>
            </div>
          </div>
