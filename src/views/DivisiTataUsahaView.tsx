...
export default function DivisiTataUsahaView({ showNotification }: any) {
  ...
  const [divisiData, setDivisiData] = useState<any>({ nama_divisi: 'Tata Usaha', nama_koordinator: 'Tim Tata Usaha' });
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const targetDiv = divList?.find((d: any) => d.nama_divisi?.toLowerCase().includes('tata usaha') || d.nama_divisi?.toLowerCase().includes('tu')) || divList?.[0];
      if (targetDiv) {
        setDivisiData(targetDiv);
        const { data: prog } = await supabase.from('program_kegiatan').select('*').eq('divisi_id', targetDiv.id);
        if (prog && prog.length > 0) { setProgramList(prog); setSelectedProgramId(prog[0].id); }
      }
  ...
  const handleSubmit = async (e: any) => {
    ...
    try {
      ...
      showNotification('Laporan Tata Usaha berhasil disimpan!', 'success');
      setCheckedItems({}); setFormInput({ petugas_pj: '', guru_target: '', catatan: '' });
      fetchProgramDetail(); setActiveSubTab('riwayat');
    } catch (err: any) { showNotification(err.message, 'error'); }
  };

  if (loading) return <div className="p-12 text-center text-slate-500 font-medium">Memuat data Divisi Tata Usaha...</div>;
  ...
  return (
    <div className="space-y-6 w-full text-left pb-12 font-sans text-slate-800">
      <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm flex justify-between items-center">
        <div>
          <h2 className="text-xl sm:text-2xl font-bold text-slate-800">Divisi Tata Usaha</h2>
      ...
      {activeSubTab === 'form' && (
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm space-y-4">
            <h3 className="font-bold text-slate-800">Informasi Layanan Administrasi Tata Usaha</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div><label className="block text-xs font-semibold text-slate-600 mb-1">Petugas TU:</label><input type="text" value={formInput.petugas_pj} onChange={e => setFormInput({...formInput, petugas_pj: e.target.value})} className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm outline-none" required /></div>
              <div><label className="block text-xs font-semibold text-slate-600 mb-1">Layanan / Pihak Terkait:</label><input type="text" value={formInput.guru_target} onChange={e => setFormInput({...formInput, guru_target: e.target.value})} className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm outline-none" required /></div>
            </div>
          </div>
          ...
          <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm space-y-4">
            <textarea value={formInput.catatan} onChange={e => setFormInput({...formInput, catatan: e.target.value})} placeholder="Catatan administrasi..." className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm outline-none h-24 resize-none" />
      ...
      {activeSubTab === 'riwayat' && (
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
          <div className="p-6 border-b border-slate-100 flex justify-between items-center"><h3 className="font-bold text-slate-800">Riwayat Tata Usaha</h3></div>
          <table className="w-full text-left text-sm whitespace-nowrap">
            <thead className="bg-slate-50 border-b border-slate-200 text-xs font-bold text-slate-500 uppercase">
              <tr><th className="px-6 py-4">Waktu</th><th className="px-6 py-4">Petugas</th><th className="px-6 py-4">Layanan</th><th className="px-6 py-4">Skor</th></tr>
            </thead>
