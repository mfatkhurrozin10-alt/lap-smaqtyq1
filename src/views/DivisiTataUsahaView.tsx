  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (dbKelas && dbKelas.length > 0) {
        ...
        setKelasList(['Pelayanan Santri', 'Kepegawaian', 'Arsip & Surat', 'Keuangan', 'Inventaris', 'Umum']);
      }
    } catch (err) {
      console.error(err);
    } finally {
          ...
          <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm space-y-4">
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              <h3 className="font-bold text-slate-800 flex items-center gap-2">
                <CheckSquare size={18} className="text-blue-600" /> Informasi Pengawasan & Petugas TU Terkait
              </h3>
              {editingLogId && (
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              {formConfig.show_guru && (
                <div>
                  <label className="block text-xs font-semibold text-slate-600 mb-1.5">Petugas TU Terkait:</label>
                  <select 
                    ...
                    required
                  >
                    <option value="">-- Pilih Petugas TU --</option>
                    {guruList.map((g: any) => (
              ...
              {formConfig.show_mapel && (
                <div>
                  <label className="block text-xs font-semibold text-slate-600 mb-1.5">Jenis Administrasi:</label>
                  <select 
              ...
              {formConfig.show_kelas && (
                <div>
                  <label className="block text-xs font-semibold text-slate-600 mb-1.5">Sektor / Bidang:</label>
                  <select 
                    ...
                    required
                  >
                    <option value="">-- Pilih Sektor / Bidang --</option>
              ...
              {formConfig.show_santri_absen && (
                <div>
                  <label className="block text-xs font-semibold text-slate-600 mb-1.5">Keterangan Dokumen:</label>
            ...
            <table className="w-full text-left text-sm whitespace-nowrap">
              <thead className="bg-slate-50 border-b border-slate-200 text-xs font-bold text-slate-500 uppercase tracking-wider">
                <tr>
                  ...
                  <th className="px-6 py-4">Petugas TU</th>
                  <th className="px-6 py-4">Administrasi & Sektor</th>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-xs p-4 overflow-y-auto">
          <div className="bg-white rounded-3xl max-w-2xl w-full p-6 shadow-2xl border border-slate-200 space-y-6 relative animate-in fade-in zoom-in duration-200">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              ...
              <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100 space-y-1">
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">ADMINISTRASI</p>
                <h4 className="text-xs font-bold text-slate-800 break-words">{selectedDetailLog.mapel_kelas}</h4>
              </div>
              <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100 space-y-1">
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">PETUGAS TU</p>
          ...
          <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm space-y-4">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3 pt-2">
              ...
              <label className={`flex items-center gap-2.5 p-3.5 rounded-xl border text-sm font-medium cursor-pointer transition-all ${formConfig.show_guru ? 'border-blue-500 bg-blue-50/20 text-blue-900 shadow-2xs' : 'border-slate-200 bg-slate-50 text-slate-600'}`}>
                <input type="checkbox" checked={formConfig.show_guru} onChange={e => handleToggleConfigField('show_guru', e.target.checked)} className="w-4 h-4 text-blue-600 rounded" /> Petugas TU
              </label>
              <label className={`flex items-center gap-2.5 p-3.5 rounded-xl border text-sm font-medium cursor-pointer transition-all ${formConfig.show_mapel ? 'border-blue-500 bg-blue-50/20 text-blue-900 shadow-2xs' : 'border-slate-200 bg-slate-50 text-slate-600'}`}>
                <input type="checkbox" checked={formConfig.show_mapel} onChange={e => handleToggleConfigField('show_mapel', e.target.checked)} className="w-4 h-4 text-blue-600 rounded" /> Administrasi
              </label>
              <label className={`flex items-center gap-2.5 p-3.5 rounded-xl border text-sm font-medium cursor-pointer transition-all ${formConfig.show_kelas ? 'border-blue-500 bg-blue-50/20 text-blue-900 shadow-2xs' : 'border-slate-200 bg-slate-50 text-slate-600'}`}>
                <input type="checkbox" checked={formConfig.show_kelas} onChange={e => handleToggleConfigField('show_kelas', e.target.checked)} className="w-4 h-4 text-blue-600 rounded" /> Sektor / Bidang
              </label>
              ...
              <label className={`flex items-center gap-2.5 p-3.5 rounded-xl border text-sm font-medium cursor-pointer transition-all ${formConfig.show_santri_absen ? 'border-blue-500 bg-blue-50/20 text-blue-900 shadow-2xs' : 'border-slate-200 bg-slate-50 text-slate-600'}`}>
                <input type="checkbox" checked={formConfig.show_santri_absen} onChange={e => handleToggleConfigField('show_santri_absen', e.target.checked)} className="w-4 h-4 text-blue-600 rounded" /> Keterangan Dokumen
              </label>
            </div>
          </div>
