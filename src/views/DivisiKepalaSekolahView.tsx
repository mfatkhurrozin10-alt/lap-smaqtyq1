...
export default function DivisiKepalaSekolahView({ showNotification }: any) {
  ...
  const [divisiData, setDivisiData] = useState<any>({
    ...
    nama_koordinator: 'Tim Kepala Sekolah'// src/views/DivisiKepalaSekolaView.tsx
import { useState, useEffect } from 'react';
import { supabase } from '../services/supabase';
import { CheckSquare, History, BarChart2, Settings, RefreshCw, Plus, Trash2, Edit, Printer, Eye, X, ChevronLeft, ChevronRight } from 'lucide-react';

export default function DivisiKepalaSekolahView({ 
  showNotification, 
  initialTab = 'form', 
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const divisiObj = divList?.find((d: any) => d.nama_divisi?.toLowerCase().includes('kepala sekolah')) || divList?.[0];
