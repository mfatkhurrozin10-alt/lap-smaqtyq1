// src/views/DivisiPerpustakaanView.tsx
...
export default function DivisiPerpustakaanView({ 
  showNotification, 
  initialTab = 'form', 
  ...
  const [divisiData, setDivisiData] = useState<any>({
    ...
    nama_koordinator: 'Tim Perpustakaan'
  });
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const divisiObj = divList?.find((d: any) => d.nama_divisi?.toLowerCase().includes('perpustakaan')) || divList?.[0];
...
const fetchProgramDetail = async () => {
  ...
  const handleSubmitCeklis = async (e: any) => {
    ...
    try {
      ...
      if (editingLogId) {
        ...
        if (showNotification) showNotification('Laporan Perpustakaan berhasil diperbarui!', 'success');
        setEditingLogId(null);
      } else {
        const { error } = await supabase.from('divisi_log_pengawasan').insert([payload]);
        if (error) throw error;
        if (showNotification) showNotification('Laporan Perpustakaan baru berhasil disimpan!', 'success');
      }
  ...
  if (loading) {
    return <div className="p-12 text-center text-slate-500 font-medium">Memuat data Divisi Perpustakaan...</div>;
  }
  ...
  return (
    <div className="space-y-6 w-full text-left pb-12 font-sans text-slate-800">
      <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h2 className="text-xl sm:text-2xl font-bold text-slate-800 tracking-tight">Divisi {divisiData?.nama_divisi || 'Perpustakaan'}</h2>
            ...
            <div className="flex items-center justify-between border-b border-slate-100 pb-4">
              <div>
                <h3 className="text-lg font-black text-slate-900 flex items-center gap-2">
                  <Eye size={20} className="text-blue-600" /> Detail Hasil Pengawasan Perpustakaan
                </h3>
