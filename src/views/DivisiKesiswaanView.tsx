...
export default function DivisiKesiswaanView({ 
  showNotification, 
  initialTab = 'form', 
  ...
  const [formInput, setFormInput] = useState({
    ...
    mapel_kelas: '',
    kelas_dipilih: '',
    jam_pembelajaran: '07:30 - Selesai',
    santri_absen: 'Nihil',
    catatan: ''
  });
  ...
  const [formConfig, setFormConfig] = useState<any>({
    ...
    label_petugas: 'Petugas / Pembina',
    label_guru: 'Target / Pembimbing',
    label_mapel: 'Kegiatan / Layanan',
    label_kelas: 'Kelas / Asrama',
    label_jam: 'Waktu Pelaksanaan',
    label_santri_absen: 'Rekap Absen / Kendala',
    type_petugas: 'input',
    type_guru: 'input',
    type_mapel: 'input',
    type_kelas: 'input',
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const divisiObj = divList?.find((d: any) => d.nama_divisi?.toLowerCase().includes('kesiswaan')) || divList?.[0];
  ...
  const handleSubmitCeklis = async (e: any) => {
    ...
    try {
      ...
      if (editingLogId) {
        ...
        if (showNotification) showNotification('Laporan kesiswaan berhasil diperbarui!', 'success');
        setEditingLogId(null);
      } else {
        const { error } = await supabase.from('divisi_log_pengawasan').insert([payload]);
        if (error) throw error;
        if (showNotification) showNotification('Laporan kesiswaan baru berhasil disimpan!', 'success');
      }
  ...
  const handleEditLog = (item: any) => {
    ...
    setFormInput({
      ...
      jam_pembelajaran: item.jam_pembelajaran || '07:30 - Selesai',
  ...
  if (loading) {
    return <div className="p-12 text-center text-slate-500 font-medium">Memuat data Divisi Kesiswaan...</div>;
  }
  ...
  return (
    <div className="space-y-6 w-full text-left pb-12 font-sans text-slate-800">
      <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h2 className="text-xl sm:text-2xl font-bold text-slate-800 tracking-tight">Divisi {divisiData?.nama_divisi || 'Kesiswaan'}</h2>
          <p className="text-sm text-slate-500 mt-0.5">Koordinator: <span className="font-semibold text-slate-700">{divisiData?.nama_koordinator || 'Tim Kesiswaan'}</span></p>
        </div>
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between bg-slate-50 p-4 rounded-2xl border border-slate-200 gap-4">
              ...
              <div className="flex items-center gap-3 w-full sm:w-auto justify-end">
                {editingLogId && (
                  <button 
                    type="button" 
                    onClick={() => { setEditingLogId(null); setFormInput({ petugas_pj: '', guru_target: '', mapel_kelas: '', kelas_dipilih: '', jam_pembelajaran: '07:30 - Selesai', santri_absen: 'Nihil', catatan: '' }); setCheckedItems({}); }}
            ...
            <div className="flex items-center justify-between border-b border-slate-100 pb-4">
              <div>
                <h3 className="text-lg font-black text-slate-900 flex items-center gap-2">
                  <Eye size={20} className="text-blue-600" /> Detail Hasil Pengawasan Kesiswaan
                </h3>
        ...
        <div className="space-y-6">
          <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 pt-2">
              {[
                { key: 'show_petugas', defaultLabel: 'Petugas / Pembina', labelKey: 'label_petugas', typeKey: 'type_petugas', optsKey: 'options_petugas' },
                { key: 'show_guru', defaultLabel: 'Target / Pembimbing', labelKey: 'label_guru', typeKey: 'type_guru', optsKey: 'options_guru' },
                { key: 'show_mapel', defaultLabel: 'Kegiatan / Layanan', labelKey: 'label_mapel', typeKey: 'type_mapel', optsKey: 'options_mapel' },
                { key: 'show_kelas', defaultLabel: 'Kelas / Asrama', labelKey: 'label_kelas', typeKey: 'type_kelas', optsKey: 'options_kelas' },
                { key: 'show_jam', defaultLabel: 'Waktu Pelaksanaan', labelKey: 'label_jam', typeKey: 'type_jam', optsKey: 'options_jam' },
                { key: 'show_santri_absen', defaultLabel: 'Rekap Absen / Kendala', labelKey: 'label_santri_absen', typeKey: 'type_santri_absen', optsKey: 'options_santri_absen' },
              ].map((item) => {
