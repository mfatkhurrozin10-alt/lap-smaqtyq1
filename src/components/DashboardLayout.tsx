  ...
  return (
    <div className="flex h-screen bg-[#f3f4f6] font-sans overflow-hidden">
      ...
      <aside className="w-72 bg-[#1e1e2f] flex flex-col h-full border-r border-slate-800 shrink-0">
        ...
        <div className="h-20 flex items-center px-6 border-b border-slate-800 gap-3 shrink-0">
          ...
          <div className="flex flex-col">
            <h1 className="font-bold text-white text-sm tracking-wide">SMP QTYQ 1 LILBANAT</h1>
            <p className="text-[10px] text-blue-300 font-semibold uppercase tracking-wider">Ledger Akademik</p>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto py-6 px-4 custom-scrollbar">
          ...
          <MenuItem id="rekap-penilaian" icon={<Users size={20} />} label="Rekap Penilaian" badge="1561" badgeColor={activeTab === 'rekap-penilaian' ? 'bg-white text-blue-600' : 'bg-amber-900 text-amber-200'} />
          <MenuItem id="presensi-absensi" icon={<UserCheck size={20} />} label="Presensi & Absensi" badge="18 Absen" badgeColor={activeTab === 'presensi-absensi' ? 'bg-white text-blue-600' : 'bg-pink-900 text-pink-200'} />
        ...
        {/* Profil Admin Bawah */}
        <div className="p-4 border-t border-slate-800 bg-[#161623] shrink-0">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-full bg-amber-500 text-amber-950 font-black text-sm flex items-center justify-center shadow-md">
              AD
            </div>
            <div>
              <p className="text-sm font-bold text-white">Administrator</p>
              <p className="text-[10px] text-amber-400 font-semibold">Akses Penuh</p>
            </div>
          </div>
        </div>
      </aside>
      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden bg-slate-50">
        ...
        {/* Topbar Simulasi Browser (Opsional, agar mirip seperti di gambar) */}
        ...
        {/* Area Konten Utama yang berganti-ganti berdasarkan state */}
